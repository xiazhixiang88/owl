...
Architecture
------------
- One long-lived process per GPU; each process loads one tile encoder.
- Each GPU owns exactly one WSI until that WSI is finished.
- Each active WSI has a CPU producer with multiple OpenSlide parser threads.
- CPU -> GPU handoff uses a bounded pinned-memory queue with backpressure.
- A dedicated CUDA copy stream prefetches the next batch while inference runs.
- WSI tasks are dynamically claimed, largest first, to reduce the long tail.
- Each GPU periodically reports live WSI progress without per-batch terminal spam.

Expected coordinate CSV columns:
    case_id,x_l0,y_l0,x_20x,y_20x,tissue_ratio

OWL/LEOPARD preprocessing:
- read 512x512 from WSI level-0 (40x)
- resize to 256x256 (20x)
- center crop to 224x224
- ImageNet normalization
- GigaPath-Flash ViT-S/16 tile encoder -> 384-d feature
"""
...
def load_coords(coord_path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    with coord_path.open("r", newline="", encoding="utf-8") as f:
        ...
        try:
            ...
            raise ValueError(f"Empty coordinate CSV: {coord_path}") from exc
...
def _read_patch_into(dest: torch.Tensor, x_l0: int, y_l0: int) -> None:
    """Decode one WSI patch directly into CHW uint8 pinned memory."""
...
def _progress_reporter(
    ...
    """Emit one lightweight progress message every `interval` seconds."""
        ...
        result_q.put(
            {
                ...
                "case_id": task.case_id,
                "message": (
                    f"summary says {task.num_tiles} tiles, coordinate CSV has {n_tiles}"
                ),
            }
        )
...
def load_tile_encoder(
    ...
    try:
        ...
        raise RuntimeError(
            "Could not import the local GigaPath-Flash tile encoder. "
            "Ensure scripts/gigapath/tile_encoder.py exists and timm is installed."
        ) from exc
...
def gpu_worker(
    ...
    try:
        ...
        while True:
            ...
            try:
                stats = run_one_wsi(task, model, device, cfg, gpu_id, result_q)
                result_q.put(
                    {
                        "type": "done",
                        "gpu": gpu_id,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--progress-interval", type=float, default=5.0, help="Seconds between live per-GPU progress reports (default: 5)")
    ...
    parser.add_argument("--checkpoint", type=Path, required=True, help="Path to local GigaPath-Flash pytorch_model.bin; network download is never attempted")
...
def format_progress(msg: dict[str, Any]) -> str:
    ...
    return (
        f"[GPU {msg['gpu']}] {msg['case_id']} | "
        f"GPU {processed:,}/{total:,} ({pct:5.1f}%) | "
...
def main() -> None:
    ...
    print(f"  progress interval:     {args.progress_interval:g}s", flush=True)
    ...
    while finished < total_tasks:
        ...
        elif msg_type == "wsi_start":
            print(
                f"[GPU {msg['gpu']}] START {msg['case_id']} | {int(msg['total']):,} tiles",
                flush=True,
            )
        elif msg_type == "progress":
            print(format_progress(msg), flush=True)
        elif msg_type == "warning":
            print(
                f"[GPU {msg['gpu']}] WARN {msg['case_id']}: {msg['message']}",
                file=sys.stderr,
                flush=True,
            )
        elif msg_type == "done":
